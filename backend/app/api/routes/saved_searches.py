...
def _get_owned_or_404(session: Session, saved_search_id: int, user: User) -> SavedSearch:
...
async def list_saved_searches(
    ...
    return session.exec(select(SavedSearch).where(SavedSearch.user_id == current_user.id)).all()
...
async def get_matches(
    ...
    saved_search = _get_owned_or_404(session, saved_search_id, current_user)
